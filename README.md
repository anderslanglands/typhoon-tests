...
pixi run pytest material-fidelity       # run materials test suite
...
pixi run pytest usdlux/iesLibPreview.usda::iesLibPreview++frame++0001 --typhoon-provider /home/anders/code/openusd-omniverse

# Run several specific frames by pytest node ID.
pixi run pytest usdlux/iesLibPreview.usda::iesLibPreview++frame++0001 usdlux/iesLibPreview.usda::iesLibPreview++frame++0002 --typhoon-provider /home/anders/code/openusd-omniverse
...
The per-run HTML report has sortable columns and defaults to render filename ascending. Status values are:
Test files may be nested to any depth under a suite. The report groups suites and their subdirectories into nested accordions, while cases directly under the suite root remain at the suite level. Directory names are used as section labels.
...
Any directory whose name begins with `_` is excluded from collection, including with `--typhoon-collect-unconfigured`. Put shared USD layers, geometry, textures, and other support files under `_assets/`. This is separate from `[skip]`, which remains available for real test cases that are intentionally unsupported.
...
- `[render].output_pattern`: expected render product path relative to the run directory. It can use `{stem}`, `{name}`, `{path}`, `{suffix}`, `{suite}`, and `{frame}` for frame-expanded cases.
...
2. Add one or more `.usda` files. Each file should author render products that write predictable filenames.
   Tests may be organized into nested section directories. Put non-test support files under `_assets/` so they are never collected.
3. Add references under `lights/reference/`. These files are hydrated locally and published with `pixi run update-references`; they are not committed directly.
...
8. Keep references deterministic. If a reference changes intentionally, update the image in the suite's `reference/` directory and run `pixi run update-references`.

## MaterialX USD Suite

The current generated suite lives under `materialx/` and is generated from `/home/anders/code/MaterialX/resources/Materials/TestSuite/_options.mtlx`. It contains 143 per-material USDA test layers plus the shared `materialx/_assets/base.usda` layer.

`materialx/_assets/base.usda` provides the shared scene:

- `/World/Sphere`: USD mesh converted from the MaterialX `sphere.obj`.
- `/World/DomeLight`: latlong IBL using `materialx/_assets/Lights/san_giuseppe_bridge.hdr`.
...
Each material test USDA sublayers `_assets/base.usda`, binds `/World/Sphere`, defines its material under `/Looks`, and sets a unique render product filename:

```text
materialx.<material-test-name>.exr
```

Referenced images and HDR maps are copied under `materialx/_assets/`, and asset paths in the USDA files point to those local copies. GLSL reference PNGs used for comparison are hydrated under `materialx/reference/` from the release manifest.
...
It contains materials derived from [bhouston/material-samples](https://github.com/bhouston/material-samples), which is licensed under the MIT License.
